<?php
final class DOMFormClass extends DOMElementClass {
    ...
    function __construct ( string $_action, string $_method = '', string $_enctype = '', string $_accept = '', string $elements = '') {
        ...
        if (empty($_method)) $this->_method = self::method_post;
        else $this->_method = $_method;

        if (empty($_enctype)) $this->_enctype = self::enctype_usual;
        else $this->_enctype = $_enctype;

        if (empty($_accept)) $this->_accept = DOMMETAClass::text_html;
        else $this->_accept = $_accept;
