<?php
if (!is_dir($websiteDir)) throw new ErrorHandler("Website wasn't found.");
...
if (!in_array($indexPage.'.'.$pagefileExtension, $pagefiles)) throw new ErrorHandler('What an error.');

$pagefiles = new ArrayClass($pagefiles);
if ($pagefiles->length(0)) throw new ErrorHandler('No pages to display.');
...
while (!$pagefiles->isEmpty()) {
	...
	if (file_exists($websiteDir.$_->value) && ($_pFileExtension === $pagefileExtension)) $this->pages->add(dirname($_->value), $_->value);
}
