	...
	foreach ($field_rules as $key =>$value) {
		...
		switch($key) {
			case (ValidationBase::VALIDATION_NULL_MYSQL): {
				if (!$data[$i] && $value === 'NO') $error->add(ValidationBase::VALIDATION_NULL_MYSQL, true);

				break;
			}
			case (ValidationBase::VALIDATION_ENUM_MYSQL): {
				if (!in_array($data[$i], $value)) $error->add(ValidationBase::VALIDATION_ENUM_MYSQL, true);
			...
			case (ValidationBase::VALIDATION_LENGTH_MYSQL): {
				if (strlen($data[$i]) > $value) $error->add(ValidationBase::VALIDATION_LENGTH_MYSQL, true);
